An input map for winit which handles mouse and keyboard buttons, mouse position and movement and the scroll wheel.
```
#[derive(ToUsize)]
enum Actions {
    Debug,
...
however, the above example wont read input in sway because no graphics are rendered.
